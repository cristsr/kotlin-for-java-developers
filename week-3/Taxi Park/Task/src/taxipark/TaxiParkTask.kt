...
fun TaxiPark.findFakeDrivers(): Set<Driver> =
    allDrivers
        .filter { trips.none { trip -> it.name == trip.driver.name } }
        .toSet()
...
fun TaxiPark.checkParetoPrinciple(): Boolean {
    ...
    val splitPercent = round((allDrivers.size * 0.20)).toInt()

    val totalCost = trips.sumOf { it.cost }

    val tripsByDriver = allDrivers
        .map { driver -> driver to trips.filter { trip -> trip.driver.name == driver.name } }
        .map { (driver, trips) -> driver to trips.sumOf { it.cost } }
        .sortedByDescending { (_, cost) -> cost }

    val twentyPercentCost = tripsByDriver.take(splitPercent).sumOf { it.second }
